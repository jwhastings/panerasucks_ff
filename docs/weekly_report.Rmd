...
position_col <- bind_cols(pos = factor(pos_lvl, levels = pos_lvl), pos_col = pos_col)
...
```{r, fig.height = 3}
matchup_pie <- head2head %>%
  inner_join(slice(matchup_simple, 1)) %>%
  left_join(position_col, by = c("pos.x" = "pos")) %>%
  left_join(position_col, by = c("pos.y" = "pos"))

fig_pie <- plot_ly() %>%
  add_pie(
    data = matchup_pie,
    labels = ~ pos.x,
    values = ~ player_score.x,
    name = ~ franchise_name.x,
    marker = list(colors = ~pos_col.x),
  ...
  add_pie(
    data = matchup_pie,
    labels = ~ pos.y,
    values = ~ player_score.y,
    name = ~ franchise_name.y,
    marker = list(colors = ~pos_col.y),
    domain = list(row = 0, column = 1),
    hole = 0.5
  ) %>%
  layout(
    title = paste0(team_x, " vs. ", team_y),
    legend = list(orientation = "h", xanchor = "center", x = 0.5),
    grid = list(rows = 1, columns = 2)
  )

div(fig_pie, align = "center")
```
...
```{r}
fig_standings <- plot_ly(weekly_records,
  x = ~week,
  y = ~weekly_rank,
...
div(fig_standings, align = "center")
```
...
```{r}
allplay_standings <- schedule %>%
  filter(week <= curr_week) %>%
  group_by(week) %>%
  expand(franchise_id, opponent_id) %>%
  filter(franchise_id != opponent_id) %>%
  left_join(select(schedule, week, franchise_id, "franchise_name" = "franchise_name.x", "fr_col" = "fr_col.x", franchise_score)) %>%
  left_join(select(schedule, week, opponent_id, opponent_score)) %>%
  mutate(
    W = if_else(franchise_score > opponent_score, 1, 0),
    L = if_else(franchise_score < opponent_score, 1, 0),
    T = if_else(franchise_score == opponent_score, 1, 0)
  ) %>%
  group_by(franchise_id, franchise_name, fr_col) %>%
  summarize(
    allplay_wins = sum(W),
    allplay_losses = sum(L),
    allplay_ties = sum(T),
    allplay_winpct = (allplay_wins + 0.5 * allplay_ties) / (allplay_wins + allplay_ties + allplay_losses)  ) %>%
  arrange(desc(allplay_winpct)) %>%
  ungroup() %>%
  mutate(rank = rank(-allplay_winpct, ties.method = "min"))
...
```{r}
lineups_qb %>%
  select(rank, franchise_name, mean_pt, all_players, n) %>%
  kbl(digits = 2,
    col.names = c("Rank", "Franchise", "Avg. Points", "Player Names", "# Players"),
    escape = F
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
...
ros <- load_rosters_weekly(curr_season) %>%
  mutate(full_name = case_when(
    gsis_id == "00-0032688" ~ "Robbie Anderson",
    TRUE ~ full_name
    )
  ) %>%
  filter(week == curr_week) %>%
...
```{r}
# Merging with ESPN
weekly_lineup <- lineups %>%
  filter(!is.na(projected_score), pos != "DST") %>%
  select(week, franchise_id, franchise_name, fr_col, player_id, player_name, clean_name, pos, team, lineup_slot, player_score) %>%
  mutate(player_id = as.character(player_id), src = "espn")

weekly_pts <- player_points %>%
  filter(week == curr_week) %>%
  select(espn_id, clean_name, everything(), -c(season, week, gsis_id, sportradar_id, player_name, pos, team))

### Step 1
# Matches by clean_name, player_id == espn_id
match_both_1 <- weekly_lineup %>%
  inner_join(weekly_pts, by = c("clean_name", "player_id" = "espn_id"))

# Non-matches
no_match_espn_1 <- weekly_lineup %>%
  anti_join(weekly_pts, by = c("clean_name", "player_id" = "espn_id"))

no_match_nflfastr_1 <- weekly_pts %>%
  anti_join(weekly_lineup, by = c("clean_name", "espn_id" = "player_id"))
...
potential_issue <- no_match_espn_2 %>% filter(player_score != 0) %>% nrow()

if (potential_issue > 0) {
  print("Possible merge issues:")
  no_match_espn_2 %>% filter(player_score != 0)
}

espn_lineup_stats <- bind_rows(match_both_1, match_both_2, no_match_espn_2)
```

### Throwin' a Fit

Teams with the most passing completions.

```{r}
most_completions <- espn_lineup_stats %>%
  filter(!(lineup_slot %in% c("BE", "IR"))) %>%
  filter(completions > 0) %>%
  group_by(franchise_id, franchise_name, fr_col) %>%
  arrange(desc(completions)) %>%
  summarize(n = n(),
            sum_compl = sum(completions),
            all_players = paste(player_name, paste0("(", completions, ")"), collapse = ", "), .groups = "keep") %>%
  ungroup() %>%
  arrange(desc(sum_compl)) %>%
  mutate(rank = rank(-sum_compl, ties.method = "min"))

most_completions %>%
  select(rank, franchise_name, sum_compl, all_players, n) %>%
  kbl(digits = 2,
    col.names = c("Rank", "Franchise", "Total Completions", "Player Names", "# Players"),
    escape = F
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  column_spec(2, background = most_completions$fr_col)
```


### Run Forest, Run!

Teams with the most rushing attempts.

```{r}
most_rushes <- espn_lineup_stats %>%
  filter(!(lineup_slot %in% c("BE", "IR"))) %>%
  filter(carries > 0) %>%
  group_by(franchise_id, franchise_name, fr_col) %>%
  arrange(desc(carries)) %>%
  summarize(n = n(),
            sum_rush = sum(carries),
            all_players = paste(player_name, paste0("(", carries, ")"), collapse = ", "), .groups = "keep") %>%
  ungroup() %>%
  arrange(desc(sum_rush)) %>%
  mutate(rank = rank(-sum_rush, ties.method = "min"))

most_rushes %>%
  select(rank, franchise_name, sum_rush, all_players, n) %>%
  kbl(digits = 2,
    col.names = c("Rank", "Franchise", "Total Carries", "Player Names", "# Players"),
    escape = F
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  column_spec(2, background = most_rushes$fr_col)
```

### Gotta Catch Em All

Teams with the most receptions.

```{r}
most_catches <- espn_lineup_stats %>%
  filter(!(lineup_slot %in% c("BE", "IR"))) %>%
  filter(receptions > 0) %>%
  group_by(franchise_id, franchise_name, fr_col) %>%
  arrange(desc(receptions)) %>%
  summarize(n = n(),
            sum_catch = sum(receptions),
            all_players = paste(player_name, paste0("(", receptions, ")"), collapse = ", "), .groups = "keep") %>%
  ungroup() %>%
  arrange(desc(sum_catch)) %>%
  mutate(rank = rank(-sum_catch, ties.method = "min"))

most_catches %>%
  select(rank, franchise_name, sum_catch, all_players, n) %>%
  kbl(digits = 2,
    col.names = c("Rank", "Franchise", "Total Receptions", "Player Names", "# Players"),
    escape = F
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  column_spec(2, background = most_catches$fr_col)
```


### Show Me Dem TDs

Team with the most touchdowns scored.

```{r}
most_tds <- espn_lineup_stats %>%
  filter(!(lineup_slot %in% c("BE", "IR"))) %>%
  mutate(total_td = passing_tds + receiving_tds + rushing_tds + kickoff_return_touchdowns + punt_return_touchdowns) %>%
  filter(total_td > 0) %>%
  group_by(franchise_id, franchise_name, fr_col) %>%
  arrange(desc(total_td)) %>%
  summarize(
    n = n(),
    sum_tot_td = sum(total_td),
    sum_pass_td = sum(passing_tds),
    sum_receiving_td = sum(receiving_tds),
    sum_rush_td = sum(rushing_tds),
    sum_st_td = sum(kickoff_return_touchdowns,  punt_return_touchdowns),
    all_players = paste(player_name, paste0("(", total_td, ")"), collapse = ", "), .groups = "keep") %>%
  ungroup() %>%
  arrange(desc(sum_tot_td)) %>%
  mutate(rank = rank(-sum_tot_td, ties.method = "min"))

most_tds %>%
  select(rank, franchise_name, sum_tot_td, sum_pass_td, sum_receiving_td, sum_rush_td, sum_st_td, all_players, n) %>%
  kbl(digits = 2,
    col.names = c("Rank", "Franchise", "Total TDs", "Pass TDs", "Receiving TDs", "Rushing TDs", "Special Team TDs", "Player Names", "# Players"),
    escape = F
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  column_spec(2, background = most_tds$fr_col)
```
